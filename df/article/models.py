...
STAR_CHOICES = [('40', '4 Stars'),
                ...
                ('05', '.5'),
                ('00', '0')]
OVERALL_CHOICES = zip(*tee(['A+', 'A', 'A-',  'B+', 'B', 'B-', 'C+',
                            'C', 'C-', 'D+', 'D', 'D-', 'F']))
...
EXPIRE_HELP = "Set an expires date for a 'continue reading' experience."
...
class Link(models.Model):
    ...
    expires = models.DateTimeField(null=True, blank=True, help_text=EXPIRE_HELP)
...
class Genre(models.Model):
    ...
    def name_to_id(cls, genre):
        """Given an id or title return the id or look up the title and return the id.
...
class Article(models.Model):
    ...
    feature = models.BooleanField(help_text='If this is a Article or Review, fill out the blurb\
                                             and check "Feature" to display on the blog.',
                                  default=False)
